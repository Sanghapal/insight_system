$A.bind(window, 'load', function(){
	...
	$A.setCalendar('BasicCalendar', $A.getEl('imgCalendar_start_date'), $A.getEl('starte_date-alt'), false, function(ev, dc, targ)
{
	...
	$A.setCalendar('AdvancedCalendar', $A.getEl('imgCalendar_end_date'), $A.getEl('end_date-alt'), false, function(ev, dc, targ){
		targ.value = dc.range.wDays[dc.range.current.wDay].lng + ' ' + dc.range[dc.range.current.month].name + ' '
			+ dc.range.current.mDay + ', ' + dc.range.current.year;
		dc.close();
	},
